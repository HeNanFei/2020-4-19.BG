...
public interface SysUserRoleInter {
    ...
    public void addSysUserRole2(@ApiParam("权限实体") SysUserRole2 sysPermission);

    @ApiOperation(value = "修改角色权限")
    @RequestMapping("/userrole/update/")
    public void updateSysUserRole2(@ApiParam("角色权限实体")  SysUserRole2 sysPermission);

    @ApiOperation(value = "列出所有用户角色")
    @ResponseBody
    @RequestMapping("/userrole/findall/")
    public List<SysUserRole2> updateSysUserRole2( );

    @ApiOperation(value = "根据id查询用户角色")
    @ResponseBody
    @RequestMapping("/userrole/find/{id}")
    public SysUserRole2 getSysUserRole2ById(@PathVariable Integer id);
}
